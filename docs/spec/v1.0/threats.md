...
running in parallel or in a subsequent environment.

*Mitigation:* Builds are [isolated] from one another, with no way for one to
affect the other or persist changes.

*Example 1 (Build L3):* Build service runs all builds for project MyPackage on
...
*Example 2 (Build L3):* Build service uses the same machine for subsequent
