...
dependencies, preferably by digest rather than version number.
...
*Mitigation:* The project owner must not have the ability to influence the build
process or provenance generation.
...
*Threat:* The package repository provides an artifact controlled by the attacker
rather than the official one.

*Mitigation:* The verifier has expectations for an authentic artifact's digest
and source repository. The attacker's artifact will not have the same digest
and/or source repository, so the inauthentic artifact will fail verification.

*Example:* MyPackage is distributed by ThePackageRepo. The authentic version
has digest `0x12345678` and is built from source repository`good/my-package`.
The adversary compromises ThePackageRepo and uploads their malicious package
with digest `0xBADDBADD` built from the source repository `evil/my-package`.
The verifier rejects the malicious artifact because the digest and source
repository do not meet its expectations.

</details>

<details><summary>Serve inauthentic provenance<span>(Verification)</span></summary>

*Threat:* The package repository provides provenance generated by an attacker
rather than the official build platform.

*Mitigation:* The verifier has a preconfigured root of trust for each build
platform it supports. The signature on the provenance's envelope layer is
derived from this root of trust. The attacker's provenance will fail
signature verification, and the verifier will reject it.

*Example:* MyPackage is distributed by ThePackageRepo. The adversary
compromises ThePackageRepo and uploads their malicious package and its
provenance. The verifier rejects the malicious artifact because its provenance
...
*Threat:* An attacker intercepts the artifact at download time and replaces it
with a malicious artifact.

*Mitigation:* The verifier has expectations for an authentic artifact's digest
and source repository. The attacker's artifact will not have the same digest
and/or source repository, so the inauthentic artifact will fail verification.

*Example:* The authentic version of MyPackage has digest `0x12345678` and is
built from source repository`good/my-package`. The adversary intercepts a
request to fetch MyPackage and responds with their malicious package
with digest `0xBADDBADD` built from the source repository `evil/my-package`.
The verifier rejects the malicious artifact because the digest and source
repository do not meet its expectations.
