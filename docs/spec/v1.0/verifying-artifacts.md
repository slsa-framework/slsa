...
verifying provenance. The second describes how to set the verification policy
used to verify provenance. The third describes the procedure for verifying an
artifact and its provenance against a verification policy.
...
provenance matches the verification policy for that package name before accepting
...
Consumers can set their own verification policy for artifacts or use default
verification policy provided by the package producer and/or package ecosystem.
In this situation, the consumer uses client-side verification tooling to ensure
that the artifact's provenance matches their verification policy for that
package before use (e.g. during installation or deployment). Client-side
verification tooling can be either standalone, such as
...
MUST publish its verification policy for all the packages it verifies.

Consumers can continuously poll a monitor to detect artifacts that
do not meet the monitor's verification policy. Detecting artifacts that fail
verification is of limited benefit unless a human or another part of the system
responds to the failed verification.

## Setting verification policy

A <dfn>verification policy</dfn> is a set of known provenance values that
indicate the corresponding artifact is authentic. For example, a package
ecosystem may maintain a mapping between package names and their canonical
source repositories. That mapping constitutes a verification policy. The package
ecosystem tooling compares artifacts' provenance against its verification policy
during upload to ensure all packages in the ecosystem are built from their
canonical source repo, which indicates their authenticity.
...
It is important to note that a verification policy is tied to a *package name*,
whereas provenance is tied to an *artifact*. Different versions of the same
package name may have different artifacts and therefore different provenance.
Similarly, an artifact may have different names in different package ecosystems
but use the same provenance file.
...
package before the package is made available to package ecosystem users.

There are several approaches a package ecosystem could take to setting a
verification policy, for example:

-   Requiring the producer to set the verification policy when registering a
    new package in the package ecosystem.
-   Using the values from the package's provenance during its initial
    publication (trust on first use).

<td>✓<td>✓<td>✓
<tr id="verification-policy-changes-auth">
<td>Changes authorized
<td>

The package ecosystem MUST ensure that any changes to the verification policy is
authorized by the package's producer. This is to prevent an adversary from
updating the verification policy to allow building and publishing from a
fork under the adversary's control. Some ways this could be achieved include:
...
-   Ensuring that the values for `BuildType` and `ExternalParameters` in the
    provenance match the verification policy. The package ecosystem MAY allow
...
[verify-step-2]: #check-verification-policy

Next, check that the package's provenance satisfies the verification policy for
that package in order to mitigate [threat "C"].
...
insider access to any trusted systems. A verification policy MUST be sufficient
to detect or prevent this adversary from injecting malicious behavior into the
package. Example threats in this category include building from an unofficial
fork or abusing a build parameter to modify the build. Usually a verification
policy identifies the canonical source repository (which is the entry in
`externalParameters`) and any other security-relevant external parameters.

The verification policy SHOULD cover the following:
...
-   **Explicit policy:** Package producer defines expectations for the
    ...
    involve two-party control over policy modifications, or having consumers
    accept each policy change (another form of trust on first use).
...
is too low. For example, `externalParameters` that record a list of commands
to run is likely impractical to verify because the commands change on every
build. Instead, consider a `buildType` that defines the list of commands in a
configuration file in a source repository, then put only the source repository
in `externalParameters`. Such a design is easier to verify because the source
