...
The producer MUST set expectations on how the package ought to be built and then
use that process on every release. How to set expectations and what build
systems are acceptable is determined by the [package ecosystem] and/or
[consumer].

### Use an acceptable build and release process
...
reaching the highest SLSA Build Levels.
