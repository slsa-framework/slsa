...
<tr>
  ...
  <td> <td> <td>✓
<tr>
  <td rowspan=2><a href="#distribution-ecosystem">Distribution ecosystem</a>
...
and what build systems are acceptable is determined by the [distribution
ecosystem] and/or [consumer]. Higher SLSA Build levels may be reached by
choosing a build system that supports that level.
...
-   Define how the package is expected to be built, if required by the
    distribution ecosystem. For example, this might require setting the upstream
    source repository in the package metadata.
...
-   Configure the release process to upload provenance generated by the build
    system, if required by the distribution ecosystem. For example, a packaging
...
If the distribution ecosystem supports verified [reproducible
...
-   Use the build and release process provided by the distribution ecosystem,
    which should generate provenance in an acceptable format.
...
The format MUST be acceptable to the [distribution ecosystem] and/or [consumer].
It is RECOMMENDED to use the [SLSA Provenance] format because it is designed to
be interoperable, universal, and unambiguous when used for SLSA. See that
format's documentation for requirements and implementation guidelines. If using
an alternate format, it MUST contain the equivalent information required at each
...
A <dfn>distribution ecosystem</dfn> is a set of conventions and tooling for
package distribution. Every package has an ecosystem, whether it is formal or
ad-hoc. Some ecosystems are formal, such as language distribution (e.g.
...
The distribution ecosystem is responsible for ensuring that consumers only use
artifacts ...**TODO**

**TODO:** Provenance MUST be available to the distribution ecosystem and/or
...
prescribed by the distribution ecosystem.
...
SLSA verification, depending on the distribution ecosystem.
