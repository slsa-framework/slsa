...
| [Source L2] | Verified history | Tampering of a repository
| [Source L3] | Two-party reviewed | Non intentional changes
...
completeness in a repository revision's provenance. Provenance describes
what Source Control Provider produced the revision, what process was used,
and who the contributors were. The lowest level only requires the provenance
to exist, while higher levels provide increasing protection against tampering
of the version control system, the provenance, or the revision.

The primary purpose of the source track is to enable verification that the
...
Organizations that are unwilling or unable to host their source on a source
control platform.
...
-   Makes easier for consumers the analysis of changes between two given
    revisions
-   Revision ID can easily and safely identify a given source revision and
    thus be used, for example, in Build provenance.
...
The source code and its change history metadata are retained and authenticated
to allow trustworthy auditing and analysis of the source code.
...
All of [Source L1], plus:

-   Strong Authentication: Changes to the source MUST be made through strong
    authentication mechanisms:
    -   Directly: Access to the source is controlled by multi-factor
        authentication or equivalent mechanism.
    -   Indirectly: New revision is signed using a signature obtained
...
-   Retained History: The revision and its change history are preserved
    ...
    the form of a provenance or audit trace.
...
Every change in the revision’s history was agreed to by two trusted entities
prior to submission, and both of these trusted entities were strongly
authenticated.
...
indicate that a file changed without showing the contents.
