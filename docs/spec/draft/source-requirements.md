...
-   Record technical controls enforced on Named References in [Source
   Provenance](#source-provenance-attestations) attestations associated with the
...
properties the SCS may wish to use. The SCS MUST enforce the use of this prefix
for such properties, unless the SCS endorses the veracity of the corresponding
claims. An organization MAY further differentiate properties using:
