...
| Source Control System (SCS) | The platform or combination of services (self-hosted or SaaS) that hosts a Source Repository and provides a trusted foundation for managing Source Revisions by enforcing policies for authentication, authorization, and change management, such as mandatory code reviews or passing status checks.
| Source Provenance | Information about how a Source Revision came to exist, where it was hosted, when it was generated, what process was used, who the contributors were, and which parent Source Revisions preceded it.
| Source Repository (Repo) | A logical unit of files, their history, as well as Branches, Tags, and other metadata.  Source Repositories are uniquely identified by a stable locator, such as a URI.
| Source Revision | A specific, logically immutable snapshot of the repository's tracked files at a point in  time. It is uniquely identified by an identifier, such as a cryptographic hash like a Git commit SHA or a path-qualified sequential number like 25@trunk/ in SVN. A Source Revision MUST include both the content (the files) and its associated version control metadata, such as the author, timestamp, and parent Source Revision(s). Source Revisions are regularly used as input to a build system (see [SLSA Build Track](build-requirements.md)). Note: Path qualification is needed for version control systems that use represent Branches and Tags using paths, such as Subversion and Perforce.
| Branch | An evolving named reference that points to a specific Source Revision within a Source Repository. It is expected to be updated to point to new Source Revisions as development occurs. Different branches may have different security requirements. Examples: `main`, `develop`, `feature-x`.
...
| Change | A proposal to update a Specific Branch to a different Source Revision. A change can be proposed and reviewed before being accepted.
| Change History | A record of the history of Source Revisions that preceded a specific Source Revision.
| Push / upload / publish | When an actor creates or modifies the Source Revisions, Branches or Tags in the Source Repository.
...
> For example, in Git, each Source Revision ID is based on the ones before it. When you remove a Source Revision, you must generate new Source Revisions (and new Source Revision IDs) for any Source Revisions that were built on top of it. Consumers who took a dependency on the old Source Revisions may now be unable to refer to the Source Revision they've already integrated into their products.
...
<td>✓<td>✓<td>✓<td>✓
<tr id="revision-ids"><td>Source Revisions are immutable and uniquely identifiable <a href="#revision-ids">🔗</a><td>
...
-   Allow organizations to distribute additional attestations related to their
   technical controls to consumers authorized to access the corresponding
   Source Revision.
...
If the SCS supports tags (or other non-branch Source Revision trackers), additional
...
SCSs that do not use cryptographic digests MUST define a canonical type that is used to identify  Source Revisions and MUST include the repository within the type[^1].
...
### Every Source Revision reachable from a Branch was approved
...
The change management tool MUST link this discussion to the Source Revision itself. This is regularly done via commit metadata.
