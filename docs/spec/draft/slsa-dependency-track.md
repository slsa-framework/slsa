...
| Dependency L4 | Dependencies meet minimum security requirements |
...
- For application-level dependencies, dependency pinning can serve as a starting point for many ecosystems as it provides identification for all transitive dependencies (e.g. `package-lock.json`, `pip –require-hashes`, `go.sum`, Gradle lock).
...
- Compliance with this level enables Level 4.
...
|-----------------|-----------------|--------|--------|--------|--------|
| Maintain an automated, complete inventory of all dependencies used in development | An organization producing artifacts MUST implement automated tooling that inventories dependencies for every version they release | ✓ | ✓ | ✓ | ✓ |
| Scan dependencies for known vulnerabilities | An organization MUST be informed when any 3rd party dependency in their build has vulnerabilities |  | ✓ | ✓ | ✓ |
