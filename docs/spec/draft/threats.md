...
that the Source Revision they received was created in the expected manner.
...
<details><summary>Software producer intentionally creates a malicious Source Revision</summary>

*Threat:* A producer intentionally creates a malicious Source Revision with the intent of harming their consumers.
...
-   The repo is open source with an active user-base. High numbers of engaged users may increase the likelihood that bad code is detected during code review and reduce the time-to-detection when bad Source Revisions are accepted.
...
`X`. Consumers of `release_1.2` will get the malicious Source Revision. Solution: The
...
Solution: Configure the code review rules to require review of the most recent Source Revision before submission.
...
*Example:* MyPackage's Source Repository requires two-party review. Adversary
...
the upstream repo.
Solution: The proposed change still requires two-party review in the upstream
context even though it received two-party review in another context.
...
*Mitigation:* The producer ensures that all Source Revisions in the protected context followed the same contribution process.
...
Solution: Each Source Revision in the protected context must have followed the intended process.
...
Source Revision is not reachable from `main`.
...
Source Revision or cause it to get garbage collected, preventing anyone from inspecting
the code.
