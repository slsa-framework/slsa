...
[verify-step-1]: #check-slsa-build-level
...
-   Establish roots of trust, meaning the recognized pairs of builder public key
    identity + `builder.id`, mapped to SLSA Build levels. Different
    organizations might use different roots of trust, but usually a verifier
    uses the same root of trust for all packages.
...
-   [Threat "H"]: Verification by the consumer covers compromise of the package
    in transit. (Many ecosystems already address this threat using package
    signatures.)
...
| What | Why |
| ---- | --- |
| Builder identity from [Step 1](verify-step-1) | To prevent an adversary from building the correct code on an unintended system |
...
Verifiers SHOULD reject unrecognized external parameters. It is acceptable to
allow a parameter to have a range of values (possibly any value) if it is known
that any value in the range is safe. Implementations need not special-case the
`buildType` if JSON comparisons are sufficient.

Possible models (not exhaustive):
...
source repository, then make the source repository the external parameter. Such
a design is easier to verify because the source repository is constant across
builds.
...
`resolvedDependencies`. A [Verification Summary Attestation (VSA)][VSA] can be
make this process more efficient by recording the result of prior verifications.
A trimming heuristic or exception mechanism will almost always be necessary
