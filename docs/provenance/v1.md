...
-   The interpretation of the other fields in `builder.`
...
<tr id="builder.tenant"><td><code>tenant</code>
<td>any<td>

A description of the tenant build project.

IMPORTANT: If the tenant is included in the trust base, it MUST also be
reflected in the `builder.id`.
...
-   Added:
    -   ResourceDescriptor:  `annotations`, `content`, `downloadLocation`,
        `mediaType`, `name`
    -   Builder: `builderDependencies`, `tenant`, and `version`
    -   `byproducts`
