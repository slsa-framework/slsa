...
-   Each build runs as an independent process on a multi-tenant build platform.
    ...
    -   The build platform implementer MUST define a security model for the build
        platform in order to clearly identify the platform's boundaries, actors,
        and interfaces. This model MUST then be used to identify the transitive
...
> ⚠ **RFC:** Do we want/need to identify the tenant of the build platform,
> separately from the build platform itself? If so, a single `id`
