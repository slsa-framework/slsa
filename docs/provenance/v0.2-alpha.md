...
artifacts as input (`materials`), and by executing certain steps (`buildConfig`).
...
invocation's `entryPoint`, `parameters`, and some of the `materials`. Finally, the
...
> `invocation.configSource`, `invocation.parameters`, `buildConfig` and
> `materials`.
...
> meaning are defined by `buildType`. For example, if the invocation type were
...
> URI indicating the identidy of the source of the config.
...
> `invocation.sourceConfig` and `invocation.environment`. For example, if the invocation
...
>
> This is an arbitrary JSON object with a schema is defined by `buildType`.
>
...
<a id="buildConfig.steps"></a>
`buildConfig.steps` __array of objects, optional_
