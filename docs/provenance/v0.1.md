...
artifacts as input (`materials`), and by executing certain steps (`recipe`).
The builder is trusted to have faithfully recorded the provenance; there is no option but to trust the builder.
However, the builder may have performed this operation at the request of some external,
possibly untrusted entity. These untrusted parameters are captured in the
invocation's `entryPoint`, `arguments`, and some of the `materials`. Finally, the
...
<a id="invocation.type"></a>
`invocation.type` _string ([TypeURI]), required_

> URI indicating what type of invocation was performed. It determines the meaning of
> `invocation.entryPoint`, `invocation.arguments`, `invocation.environment`, and
> `materials`.

<a id="invocation.definedInMaterial"></a>
`invocation.definedInMaterial` _integer, optional_

> Index in `materials` containing the invocation steps that are not implied by
> `invocation.type`. For example, if the invocation type were "make", then this would
> point to the source containing the Makefile, not the `make` program itself.
>
> Omit this field (or use null) if the invocation doesn't come from a material.
>
> TODO: What if there is more than one material?

<a id="invocation.entryPoint"></a>
`invocation.entryPoint` _string, optional_

> String identifying the entry point into the build. This is often a path to a
> configuration file and/or a target label within that file. The syntax and
> meaning are defined by `invocation.type`. For example, if the invocation type were
...
> point.
>
> MAY be omitted if the invocation type specifies a default value.
>
...
<a id="invocation.arguments"></a>
`invocation.arguments` _object, optional_

> Collection of all external inputs that influenced the build on top of
> `invocation.definedInMaterial` and `invocation.entryPoint`. For example, if the invocation
> type were "make", then this might be the flags passed to `make` aside from the
> target, which is captured in `invocation.entryPoint`.
>
> Consumers SHOULD accept only "safe" `invocation.arguments`. The simplest and
> safest way to achieve this is to disallow any `arguments` altogether.
>
> This is an arbitrary JSON object with a schema is defined by `invocation.type`.
>
...
> invocation. Usually only needed for [reproducing][reproducible] the build but not
...
> actually affect the evaluation of the invocation. For example, this might include
...
> If true, the `builder` claims that `invocation.arguments` is complete, meaning
> that all external inputs are propertly captured in `invocation`.
...
> URI indicating what type of recipe was performed. It determines
> the meaning of `recipe.steps`

<a id="recipe.steps"></a>
`recipe.steps` __array of objects, optional_

> List of steps the build system ran including base images, environment 
> variables, and scripts run.

> This is an arbitrary JSON object with a schema defined by `recipe.type`.
...
-   Separated `recipe` into `invocation` and `recipe.steps`
