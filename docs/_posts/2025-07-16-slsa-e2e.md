...
We’re looking for examples that document the entire end-to-end story with
working implementations that people can try for themselves. These should be
shareable either directly with the SLSA community as indicated later but may
also contain content outside of the SLSA repository (i.e. linked demo repositories
...
-   **Collect source VSAs for all used source repos (as listed in resolvedDendencies):** For each source,
  retrieve and store its VSA to verify its origin and integrity.
    ...
    [verify the validity of each dependency's source VSA](/spec/v1.2-rc1/verifying-source#how-to-verify-slsa-a-source-revision)
