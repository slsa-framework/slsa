...
`vsa.policyLevel` and `vsa.dependencyLevels`.  Policy verifiers wishing
...
<a id="policyLevel"></a>
`policyLevel` _string ([SlsaResult]), required_

> Indicates which SLSA tracks and levels the artifact itself (and not its
> dependencies) were verified at or "FAILED" if policy verification failed.

<a id="dependencyLevels"></a>
`dependencyLevels` _object, required_
