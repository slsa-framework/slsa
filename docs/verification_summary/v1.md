...
<a id="policy_level"></a>
`policy_level` _object ([SlsaResult]), required_

> Map from _string to [SlsaResult]. Indicates for each track what SLSA level the
> artifact itself (and not its dependencies) was verified at or "FAILED" if
...
-   1:
    ...
    -   Added `policy_levels.build` field.
-   0.2:
