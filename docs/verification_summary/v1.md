...
They also allow software producers to keep the details of their build pipeline
...
dependencies all at once, the verifier can verify each one independently and
produce VSAs. Finally, the verifier combines those VSAs, with the artifact acting
as the final VSA's `subject` and each transitive dependency being an entry in
`dependencyLevels`.
