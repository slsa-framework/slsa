...
The Dependency Track defines the *shape of evidence* for ingested
dependencies in that reality. It does not prescribe operational best
practices (which scanners to run, which packages to allow, what
version-age cutoff to use). It defines the Dependency Ingestion
Provenance — a per-ingestion attestation describing how each dep
entered the ingestor's environment — and the architectural properties
needed to make that Provenance trustworthy: a controlled ingestion
path, identity verification, signed evidence, and at L3 a platform
hardened against subversion by hostile dependencies.

The three modes by which an ingestor establishes trust in an ingested
dependency are described in
[Modes of establishing trust](#modes-of-establishing-trust). Each
mode is recorded in the Provenance so downstream verifiers can apply
their own policy to decide whether the recorded set is acceptable.

Attestations from the ingestor producing their own producer-side
evidence over consumed deps (via rebuild) are deferred to a future
cross-cutting axis. See [Future Considerations](#future-considerations).
...
### Mode 1: Verified upstream attestation

The dependency comes with a signed attestation from an upstream
identity the ingestor trusts. Examples include a SLSA Build
[Provenance](provenance.md), a
[Verification Summary Attestation (VSA)](verification_summary.md), or
the upstream publisher's own signature on the artifact. The
platform verifies the attestation against the ingestor's
expectations and records the verification verdict.
...
### Mode 2: Trusted distribution channel

The dependency comes from a distribution system that downstream
verifiers already trust — an internal package registry that applies
its own due-diligence at admission, a curated feed known to admit
only vetted software, or a distribution bundle maintained under the
ingestor's control. The ingestor's trust flows from *the distribution
system's identity and reputation*, not from a per-artifact upstream
attestation.

In the Provenance: [`ingestionPlatform.id`](dependency-provenance.md)
and the ingestion path. Downstream verifiers refuse Provenance from
platform identities they do not trust — analogous to how a verifier
of SLSA Build Provenance trusts specific `builder.id` values.

Mode 2 typically *composes with Mode 3 recursively*: the trusted
distribution system itself applied its own admission process (Mode 3)
when it, in turn, ingested from upstream. The end-consumer of the
Provenance trusts the distribution system's identity; the
distribution system's own Provenance chain records how it, in turn,
established trust.

### Mode 3: Ingestor's own admission process

The ingestor applies their own organizational policies at admission
time — a security-team review, an allow-list match, a custom malware
sweep, a two-person approval workflow, or any other process the
organization runs. Trust flows from *the ingestor's own controls*.

In the Provenance:
[`policyEvaluations[]`](#dep-policy-attestation), one entry per
policy the platform ran.
...
integrity check, none of the three trust arguments can be sure the
bytes admitted match the bytes any of the modes was reasoning about.
This is why the identity-verification (integrity) verdict is a
track-level requirement at L2+ regardless of which mode(s) the
ingestor uses.

### Modes and levels

The trust modes and the L1 / L2 / L3 axis answer different questions:

-   **The modes** answer *how did the ingestor decide to trust this
    dep?* — a claim about the dep.
-   **The levels** answer *how trustworthy is the ingestor's claim?*
    — a claim about the Provenance itself.

At L1 the Provenance MAY be unsigned. Mode 2 (trust the platform's
identity) offers little on its own at L1 because the platform's
identity claim in an unsigned Provenance is not independently
verifiable. Mode 1 with independent upstream verification remains
meaningful at L1 because the upstream attestation carries its own
signature.

At L2, the Provenance is signed by the platform. All three modes
become verifiable — Mode 2 gains meaning because the platform's
identity is now cryptographically bound to the Provenance.

At L3, the Provenance is unforgeable by hostile deps. All three modes
gain the strongest form of the same signature-backed guarantees; a
malicious dep cannot subvert its own trust claim.

## What SLSA does not claim about ingested dependencies

The Dependency Track does not judge whether the ingestor's chosen
...
used, the modes are transparently recorded in the Dependency
Ingestion Provenance, the Provenance is signed by the platform (at
L2+), and at L3 the Provenance cannot be subverted by a hostile
dependency during ingestion*. Downstream verifiers apply their own
policy to decide whether the recorded set of modes, verdicts, and
identities satisfies their acceptance criteria.

This is the direct analog of the SLSA Build Track's stance: Build L3
does not prevent a producer from performing a risky or insecure
build; it guarantees that whatever the build did is faithfully
recorded in signed Provenance the build tenant cannot forge. The
Dependency Track is the same for ingestion.
