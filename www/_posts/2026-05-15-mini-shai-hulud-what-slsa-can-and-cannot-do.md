...
L3 isolation is a platform guarantee, not a build process guarantee. The spec is explicit: there are no requirements on the build itself. Build L3 ensures that a well-intentioned build runs securely, not that every build is well-intentioned. Verifying that the build did what was intended still requires examining the provenance properties: what source was built, which parameters were declared, which steps ran. Isolation makes those observations trustworthy; evaluating them is the consumer's responsibility.
...
SLSA also does not evaluate whether a workflow's trigger configuration is safe, whether its permissions are least-privileged, or whether its caching strategy creates cross-workflow trust boundaries. The `pull_request_target` misconfiguration that initiated this attack falls entirely outside SLSA's scope: it is a workflow authoring error, not a build platform integrity failure. This is also consistent with how L3 isolation is scoped: the spec explicitly imposes no requirements on the build process itself. An isolated build platform faithfully records what a misconfigured workflow does; it cannot distinguish intended behavior from attacker-controlled behavior inside that workflow.
